...
type SeriesKey =
  | (typeof BASE_SERIES_KEYS)[number]
  | 'cacheCreationTokens';
...
type TokenTrendPoint = UsageTrendChartPoint & {
    ...
    return source.map((row) => {
      ...
      return {
        ...
        otherTokens: Math.max(
          0,
          row.totalTokens - inputTokens - row.cachedInputTokens - row.outputTokens,
        ),
        totalTokens: row.totalTokens,
        costUsd: row.costUsd,
      };
...
function TokenDayTooltip({
  ...
  active?: boolean;
  payload?: Array<{ payload?: TokenTrendPoint }>;
}) {
        ...
        {seriesKeys.map((key) => (
          ...
          >
            <span className="flex items-center gap-1.5 text-muted">
              ...
              />
              {seriesKeys.includes('cacheCreationTokens') &&
