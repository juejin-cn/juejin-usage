...
test('converts used allowance into a bounded remaining percentage', () => {
  assert.equal(codexRemainingPercent(0), 100);
  assert.equal(codexRemainingPercent(70), 30);
  assert.equal(codexRemainingPercent(140), 0);
  assert.equal(codexRemainingPercent(Number.NaN), 0);
});
